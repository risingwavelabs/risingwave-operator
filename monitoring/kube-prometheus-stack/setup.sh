...
ZOOKEEPER_NAME="zkp"
KAFKA_NAME="kstack"
KAFKA_BROKER_URI="kstack-kafka-headless.risingwave-operator-system:9092"
...
if [ ! -z "${namespace}" ]; then
    ...
    rwc tenant delete -name "${MONITORING_TENANT_NAME}"
    echo "cleaning up existing resources..."
    kubectl wait --for delete pods --all -n "${namespace}" --timeout=5m
    if [ $? -ne 0 ]; then
...
psql "$(rwc tenant endpoint -name "${MONITORING_TENANT_NAME}")" -- c <<EOF
    ...
    kafka.brokers = '${KAFKA_BROKER_URI}',
    kafka.consumer.group = '${KAFKA_CONSUMER_GROUP}', kafka.scan.startup.mode = 'earliest' ) ROW FORMAT JSON;
EOF
