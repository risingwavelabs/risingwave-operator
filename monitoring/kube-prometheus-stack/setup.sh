...
if [ "$(rwc t get -tenant "${MONITORING_TENANT_NAME}" 2>&1 | grep '401')" != "" ]; then
    echo "Login required..."
...
if [ "$(kubectl get pods --namespace "${MONITORING_TENANT_NAME}")" != "" ]; then
    echo "existing monitoring tenant detected. cleaning up"
    kubectl delete --all pods --namespace="${MONITORING_TENANT_NAME}"
    kubectl delete --all deployments --namespace="${MONITORING_TENANT_NAME}"
    echo "cleaning up existing resources..."
    kubectl wait --for delete --all pods --namespace="${MONITORING_TENANT_NAME}" --timeout=5m
    kubectl wait --for delete --all deployments --namespace="${MONITORING_TENANT_NAME}" --timeout=5m
    if [ $? -ne 0 ]; then
        echo "Could not delete all deployments in namespace ${MONITORING_TENANT_NAME}"
        exit 1 
    fi
fi
